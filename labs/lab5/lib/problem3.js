...
const isValidNumber = (input) => {
  ...
  // ใช้ isFinite เพื่อตรวจสอบให้ครบถ้วนเพิ่มเติม เช่น isFinite("abc"); = false
};
...
const validateNumberInput = (input) => {
  ...
  const numericValue = parseValidNumber(input);
...
const performCalculationOnNumbers = function (numbers, operationCallback) {
  ...
  // เช็คว่ามีตัวเลขถูกเพิ่มเข้าไปไหม
  if (!numbers || numbers.length === 0) {
    throw new Error("No numbers provided for calculation");
  }
  
  // ถ้ามีตัวเลขเดียวให้ return ตัวนั้นเลย
  if (numbers.length === 1) {
    return numbers[0];
  }
...
const calculateWithCallback = function (numbers, operation) {
  ...
  return performCalculationOnNumbers(numbers, operationFunction);
};
...
function handleAddNumber() {
  ...
  if (validation.valid) {
    ...
    // ✅ แก้ไข: แสดงข้อความตาม requirement
